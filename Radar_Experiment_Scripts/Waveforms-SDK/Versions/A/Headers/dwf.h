...
const DEVID devidDiscovery3 = 10;
const DEVID devidADP2230    = 14;
...
const DwfAnalogOutIdle DwfAnalogOutIdleHold     = 3;
...
const DwfParam DwfParamDigitalVoltage  = 13; // mV
const DwfParam DwfParamFreqPhaseSteps  = 14; // readonly
...
const DwfWindow DwfWindowBlackman       = 8;
const DwfWindow DwfWindowFlatTopM       = 9;
...
// FIR and IIR filters
typedef int DwfFiirMode;
const DwfFiirMode DwfFiirWindow         = 0;
const DwfFiirMode DwfFiirFir            = 1;
const DwfFiirMode DwfFiirIirButterworth = 2;
const DwfFiirMode DwfFiirIirChebyshev   = 3;

typedef int DwfFiirType;
const DwfFiirType DwfFiirLowPass    = 0;
const DwfFiirType DwfFiirHighPass   = 1;
const DwfFiirType DwfFiirBandPass   = 2;
const DwfFiirType DwfFiirBandStop   = 3;

typedef int DwfFiirInput;
const DwfFiirInput DwfFiirRaw      = 0;
const DwfFiirInput DwfFiirDecimate = 1;
const DwfFiirInput DwfFiirAverage  = 2;
...
DWFAPI int FDwfAnalogInStatusDataMix16(HDWF hdwf, short *rgu16Data, int idxData, int cdData);
...
DWFAPI int FDwfAnalogInBuffersInfo(HDWF hdwf, int *pMax);
DWFAPI int FDwfAnalogInBuffersSet(HDWF hdwf, int n);
DWFAPI int FDwfAnalogInBuffersGet(HDWF hdwf, int *pn);
DWFAPI int FDwfAnalogInBuffersStatus(HDWF hdwf, int *pn);
...
// FIR and IIR filters
DWFAPI int FDwfAnalogInChannelFiirInfo(HDWF hdwf, int idxChannel, int *cFIR, int *cIIR);
DWFAPI int FDwfAnalogInChannelFiirSet(HDWF hdwf, int idxChannel, DwfFiirInput input, DwfFiirMode fiir, DwfFiirType pass, int ord, double hz1, double hz2, double ep);
DWFAPI int FDwfAnalogInChannelWindowSet(HDWF hdwf, int idxChannel, DwfWindow win, int size, double beta);
DWFAPI int FDwfAnalogInChannelCustomWindowSet(HDWF hdwf, int idxChannel, const double *rg, int size, int normalize);
...
DWFAPI int FDwfDigitalIODriveInfo(HDWF hdwf, int channel, double *ampMin, double *ampMax, int *ampSteps, int *slewSteps);
...
DWFAPI int FDwfDigitalInStatusData3(HDWF hdwf, void *rgData, int idxSample, int countOfDataBytes, int bitShift);
DWFAPI int FDwfDigitalInStatusNoise2(HDWF hdwf, void *rgData, int idxSample, int countOfDataBytes);
DWFAPI int FDwfDigitalInStatusNoise3(HDWF hdwf, void *rgData, int idxSample, int countOfDataBytes, int bitShift);
...
DWFAPI int FDwfDigitalInBuffersInfo(HDWF hdwf, int *pMax);
DWFAPI int FDwfDigitalInBuffersSet(HDWF hdwf, int n);
DWFAPI int FDwfDigitalInBuffersGet(HDWF hdwf, int *pn);
DWFAPI int FDwfDigitalInBuffersStatus(HDWF hdwf, int *pn);
...
DWFAPI int FDwfDigitalInTriggerInfo64(HDWF hdwf, unsigned long long *pfsLevelLow, unsigned long long *pfsLevelHigh, unsigned long long *pfsEdgeRise, unsigned long long *pfsEdgeFall);
DWFAPI int FDwfDigitalInTriggerSet64(HDWF hdwf, unsigned long long fsLevelLow, unsigned long long fsLevelHigh, unsigned long long fsEdgeRise, unsigned long long fsEdgeFall);
DWFAPI int FDwfDigitalInTriggerGet64(HDWF hdwf, unsigned long long *pfsLevelLow, unsigned long long *pfsLevelHigh, unsigned long long *pfsEdgeRise, unsigned long long *pfsEdgeFall);
...
DWFAPI int FDwfDigitalInTriggerResetSet64(HDWF hdwf, unsigned long long fsLevelLow, unsigned long long fsLevelHigh, unsigned long long fsEdgeRise, unsigned long long fsEdgeFall);
...
DWFAPI int FDwfDigitalUartParitySet(HDWF hdwf, int parity); // 0 no parity, 1 even, 2 odd, 3 mark (high), 4 space (low)
...
DWFAPI int FDwfSpectrumGoertzel(const double *rgdData, int cdData, double pos, double *pMag, double *pRad);
