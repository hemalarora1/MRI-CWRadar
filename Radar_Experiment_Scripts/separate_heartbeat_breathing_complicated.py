...
# file_path = 'Data/081824/KC_frbr5_tweaking_RCs/POST_SSA_Data_RCs2-20_Chest.csv'

# Semi-Successful filepaths for separating breathing and heartbeat:
# file_path = 'Data/081824/KC_frbr1/POST_SSA_Data_Chest.csv'
# file_path = 'Data/081824/KC_frbr1_tweaking_RCs/POST_SSA_Data_RCs4-20_Chest.csv'
...
# Step 3: Filtering for Cardiac Signal Isolation
...
# Apply bandpass filter to isolate cardiac-related signals
...
ecg_filtered = bandpass_filter(ecg, lowcut, highcut, fs)

# Step 4: Frequency Spectrum Analysis for both Raw and Filtered Signals
def compute_frequency_spectrum(signal, fs):
    freqs, psd = periodogram(signal, fs)
    return freqs, psd

# Compute frequency spectra for raw and filtered ECG, Left Radar, and Right Radar
...
freqs_ecg_filtered, psd_ecg_filtered = compute_frequency_spectrum(ecg_filtered, fs)
freqs_left_filtered, psd_left_filtered = compute_frequency_spectrum(left_radar_filtered, fs)
freqs_right_filtered, psd_right_filtered = compute_frequency_spectrum(right_radar_filtered, fs)
...
snr_left = compute_snr(left_radar_filtered, fs, lowcut, highcut)
snr_right = compute_snr(right_radar_filtered, fs, lowcut, highcut)

# Print SNR values
print(f"SNR (Left Radar): {snr_left:.2f} dB")
print(f"SNR (Right Radar): {snr_right:.2f} dB")
...
# Frequency Spectrum of Raw Signals
...
axs[0, 2].set_title('Frequency Spectrum of Raw Signals')
axs[0, 2].set_xlim([0, 5])
axs[0, 2].legend()

# Frequency Spectrum of Filtered Signals
axs[1, 2].plot(freqs_ecg_filtered, psd_ecg_filtered, label='ECG (Filtered)', color='red')
axs[1, 2].plot(freqs_left_filtered, psd_left_filtered, label='Left Radar (Filtered)', color='blue')
axs[1, 2].plot(freqs_right_filtered, psd_right_filtered, label='Right Radar (Filtered)', color='green')
axs[1, 2].set_xlabel('Frequency (Hz)')
axs[1, 2].set_ylabel('Power Spectral Density')
axs[1, 2].set_title('Frequency Spectrum of Filtered Signals')
